...
function spawnBoss(bossType) {
  ...
  if (bossType === 'titan') {
    bosses.push({ id: 'titan_boss', name: '🔴 泰坦巨獸 (TITAN DREADNOUGHT)', x: ex, y: ey, hp: 500 + hpBonus, maxHp: 500 + hpBonus, speed: 1.1, radius: 42, color: '#dc2626', stunned: 0, slowed: false, skillTimer: 240, enrageTimer: 0 });
  } else if (bossType === 'void') {
    bosses.push({ id: 'void_boss', name: '🟣 虛空領主 (VOID STALKER)', x: canvas.width / 2 + (Math.random() * 200 - 100), y: -60, hp: 380 + hpBonus, maxHp: 380 + hpBonus, speed: 2.2, radius: 32, color: '#9333ea', stunned: 0, slowed: false, skillTimer: 170 });
